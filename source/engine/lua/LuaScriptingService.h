...
public:   
    ...
    void LuaPushIntegral(const long long value);

    /// Pushes a double value to the Lua stack.
    ///
    /// @param[in] value value to be pushed to the Lua stack.
    void LuaPushDouble(const double value);
    ...
    void LuaCallGlobalFunction(const std::string& globalFunctionName);
    ...
    void LuaCallGlobalFunction(const std::string& globalFunctionName, const int argc, const Arg& firstArg, Args... args)
    {
...
private:    
    ...
    void LuaGetGlobal(const std::string& globalName);

    // Internal lua proc call wrapper.
    void LuaProcedureCall(const int argc);

    // Helper Push Argument specializations.
    template<class T, class... Args>
    void LuaPushArgument(const T& t, Args... args)
    {
        LuaPushArgument(t);
    ...
    template<class T>
    typename std::enable_if<std::is_same<T, std::string>::value>::type LuaPushArgument(const T& t)
    {
        LuaPushString(t);
    }

    template<std::size_t N>
    void LuaPushArgument(const char(&value)[N])
    {
        LuaPushString(value);
    }

    template<class T>
    typename std::enable_if<std::is_integral<T>::value>::type LuaPushArgument(const T& t)
    {
        LuaPushIntegral(t);
    }

    template<class T>
    typename std::enable_if<std::is_floating_point<T>::value>::type LuaPushArgument(const T& t)
    {
        LuaPushDouble(t);
    }    
